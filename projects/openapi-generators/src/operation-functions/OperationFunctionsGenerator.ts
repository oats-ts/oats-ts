...
import { OperationFunctionLocals, OperationFunctionsGeneratorConfig } from './typings'
import {
  EnhancedOperation,
  hasRequestBody,
  hasResponseHeaders,
...
export class OperationFunctionsGenerator extends OperationBasedCodeGenerator<OperationFunctionsGeneratorConfig> {
  public name(): OpenAPIGeneratorTarget {
    return 'oats/operation-function'
  }
  ...
  protected async generateItem(item: EnhancedOperation): Promise<Try<SourceFile>> {
    const path = this.context().pathOf(item.operation, 'oats/operation-function')
  ...
  protected getOperationFunctionAst(data: EnhancedOperation): Statement {
    ...
    const params: ParameterDeclaration[] = [
      ...(isNil(requestType)
        ...
        : [
            factory.createParameterDeclaration(
              ...
              [],
              undefined,
              this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'request'),
              undefined,
              requestType,
            ),
          ]),
      factory.createParameterDeclaration(
        ...
        [],
        undefined,
        this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'adapter'),
        undefined,
    ...
    const node = factory.createFunctionDeclaration(
      ...
      undefined,
      this.context().nameOf(operation, 'oats/operation-function'),
      [],
      params,
          ...
          factory.createVariableDeclaration(
            ...
            factory.createAwaitExpression(
              factory.createCallExpression(
                ...
                [
                  factory.createIdentifier(
                    this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'rawRequest'),
                  ),
                ],
              ),
            ),
  ...
  protected getRawRequestStatement(data: EnhancedOperation): Statement | undefined {
    const urlNames: [string, string] = [
      ...
      this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'requestUrl'),
    ]
    ...
    const bodyNames: [string, string] = [
      ...
      this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'requestBody'),
    ]
    const headerNames: [string, string] = [
      RawHttpRequestFields.headers,
      this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'requestHeaders'),
    ]
              ...
              factory.createCallExpression(
                ...
                [
                  ...
                  data.cookie.length > 0 && this.configuration().sendCookieHeader
                    ? factory.createIdentifier(
                        this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'cookies'),
                      )
                    : factory.createIdentifier('undefined'),
                  data.header.length > 0
              ...
              factory.createCallExpression(
                ...
                [
                  data.path.length === 0
                    ...
                    : factory.createIdentifier(
                        this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'path'),
                      ),
                  data.query.length === 0
                    ? factory.createIdentifier('undefined')
                    : factory.createIdentifier(
                        this.context().localNameOf<OperationFunctionLocals>(undefined, this.name(), 'query'),
                      ),
                ],
              ),
            ),
